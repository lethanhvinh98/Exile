...
void File(int highScore)
{
	system("cls");
	HienConTro();
	vector<Score> topScore;

	ifstream FileIn("fileData\\highScore.in", ios::in); //Mở file hight score;
	if (FileIn.fail()) // Thất bại => lần đầu play;
	{
		SCORE s;
		ofstream FileOut("fileData\\highScore.in", ios::out); // Tạo mới file hight socre;
		do {
	...
	ofstream FileOut("fileData\\highScore.in", ios::out);
	FileOut.clear();
